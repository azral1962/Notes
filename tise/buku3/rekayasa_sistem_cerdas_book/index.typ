...
Secara formal, urutan pengerjaan untuk merekayasa sistem lingkungan hidup digital bagi lansia (dengan layar #emph[touch screen], avatar cerdas, dan dukungan eksternal) akan mengikuti kerangka kerja #strong[Smart Engineering] yang membagi penyelesaian masalah ke dalam empat domain rekayasa (A, S, T, F), digabungkan dengan alur kerja integrasi sistem cerdas.
...
#strong[Langkah 1: Perumusan Domain Aplikasi (Application Layer)] Langkah pertama berfokus pada definisi masalah dan kebutuhan pengguna akhir. \* #strong[Identifikasi Stakeholder (P):] Lansia sebagai pengguna utama, serta keluarga atau perawat sebagai pengguna dukungan eksternal. \* #strong[Definisi Masalah (Cx) & Outcome (O):] Mengidentifikasi kendala lansia (misalnya kesulitan mobilitas, kesepian, atau kebutuhan pengingat medis) dan menetapkan metrik keberhasilan, seperti peningkatan kesejahteraan atau efisiensi respons darurat. \* #strong[Intervensi (I):] Merumuskan konsep solusi, yakni asisten virtual berbasis layar sentuh dengan avatar yang ramah bagi lansia.

#strong[Langkah 2: Perancangan Arsitektur Sistem (System Layer) dengan Siklus PUDAL] Insinyur merancang sistem utama dengan memastikan bahwa avatar dan sistem pemrosesannya berfungsi sebagai #strong[Sistem Cerdas (#emph[Intelligent System])] yang terus menjalankan siklus #strong[PUDAL]: \* #strong[Perceive (Persepsi):] Sensor pada layar sentuh atau mikrofon menerima input atau perintah dari lansia. \* #strong[Understand (Memahami):] Sistem memproses data masukan (misal: "Saya merasa pusing") untuk memahami konteks dan kondisi lansia saat itu. \* #strong[Decision-making & Planning (Keputusan):] Berdasarkan aturan kesehatan, sistem memilih tindakan (misal: apakah cukup menenangkan melalui avatar, atau harus mengontak dukungan eksternal/perawat darurat). \* #strong[Act-Response (Tindakan):] Layar mengeksekusi tindakan, di mana avatar memberikan instruksi visual dan suara, sementara sistem internal mengirim peringatan ke pihak eksternal. \* #strong[Learning-Evaluating (Pembelajaran):] Sistem mengevaluasi keefektifan respons dan mempelajari pola kebiasaan lansia untuk pelayanan masa depan.

#strong[Langkah 3: Pemilihan Mesin/Modul dalam Domain Teknologi (Technology Layer)] Pada tahap ini, teknologi dipilih untuk mewujudkan #emph[Smart Engine] yang memisahkan logika dari aplikasi. \* #strong[Python sebagai Orkestrator:] Python akan digunakan untuk membangun antarmuka grafis (#emph[User Interface]) layar sentuh, merender animasi avatar, serta mengelola koneksi API jaringan untuk komunikasi dengan layanan dukungan eksternal (rumah sakit/keluarga). \* #strong[Prolog sebagai Otak Penalaran:] Prolog digunakan di latar belakang sebagai basis pengetahuan (#emph[knowledge base]) yang menyimpan aturan-aturan medis atau logika keseharian (misalnya aturan kapan harus minum obat, atau logika eskalasi situasi gawat). \* #strong[Pustaka Penghubung (#emph[Bridge Library]):] Teknologi seperti PySWIP atau Janus digunakan untuk menyambungkan Python (layar/avatar) dengan Prolog (logika) agar pertukaran data terjadi dalam hitungan mikrodetik.

#strong[Langkah 4: Rekayasa Ontologi dan Riset Fundamental (Fundamental Layer)] Sebelum sistem diprogram, insinyur harus membangun ontologi kehidupan lansia dan mentransformasikannya menjadi basis pengetahuan yang dapat dipahami mesin. \* #strong[Pengembangan Basis Pengetahuan:] Mendefinisikan fakta dan aturan (#emph[facts and rules]) di dalam #emph[file] Prolog. Misalnya, membuat relasi ontologis #NormalTok("jadwal_obat(lansia_A, pagi)"); atau aturan #NormalTok("kondisi_darurat(X) :- detak_jantung(X, tinggi), usia(X, lansia).");. \* #strong[Desain Konversi Transaksional PSKVE:] Memastikan ada keseimbangan energi. Misalnya, lansia memberikan #strong[Energi Layanan] (waktu dan interaksi dengan layar) dan sistem memberikan #strong[Energi Pengetahuan/Nilai] (bantuan hidup yang berharga melalui interaksi avatar).

#strong[Langkah 5: Integrasi, Orkestrasi, dan Simulasi Eksekusi] Langkah eksekusi akhir di mana seluruh komponen dirakit dan disimulasikan. \* #strong[Inisialisasi:] Aplikasi Python (layar sentuh) dinyalakan dan langsung memuat kode ontologi Prolog ke dalam memori. \* #strong[Modifikasi Fakta Dinamis:] Saat lansia berinteraksi dengan avatar atau sistem mendeteksi waktu tertentu, Python menyuntikkan (#emph[assert]) status terbaru ke dalam Prolog (misal: #NormalTok("telah_minum_obat(lansia_A, pagi)");). \* #strong[Kueri Berkelanjutan:] Saat layar menerima input darurat, Python mengirimkan kueri string ke mesin Prolog. Prolog mengevaluasi kondisinya dan mengirim jawaban kembali ke Python. \* #strong[Respons Antarmuka:] Python menerjemahkan hasil dari Prolog untuk menggerakkan avatar agar memberikan respons verbal yang tepat di layar, atau memicu skrip eksternal untuk menelepon ambulans.

#heading(level: 1, numbering: none)[Bagian I : Konsep Inti Ontologi]
...
- #strong[Pada Proses Rekayasa (Tim Insinyur):] Proses perancangan (#emph[engineering process]) yang dilakukan oleh manusia juga dipandang sebagai sebuah sistem cerdas yang menjalankan siklus menyerupai PUDAL. Dalam konteks ini, tahap "Perceive" terjadi ketika #strong[para insinyur mempersepsikan atau menangkap kebutuhan pengguna (#emph[user needs]) beserta batasan-batasan (#emph[constraints]) dari masalah yang ada]. Persepsi awal dari insinyur ini menjadi landasan untuk memahami domain masalah sebelum merancang dan mengimplementasikan artefak.
...
- #strong[Pada Proses Rekayasa (Tim Insinyur Manusia):] Dalam memandang proses kerja insinyur sebagai sebuah sistem cerdas, tahap #emph[Act] merupakan transisi dari sekadar konsep di atas kertas menjadi realitas fisik. Para insinyur "bertindak" melalui #strong[kegiatan implementasi teknis dan pembuatan purwarupa (#emph[prototyping])]. Mereka mengeksekusi desain yang telah diputuskan sebelumnya untuk menciptakan artefak rekayasa yang nyata.
...
- #strong[Pada Proses Rekayasa (Tim Insinyur):] Proses desain rekayasa yang dilakukan manusia juga mengandalkan tahap evaluasi ini. Bagi para insinyur, fase #emph[Learning] terjadi ketika mereka #strong[mengevaluasi hasil pengujian (#emph[testing]) dan mengumpulkan umpan balik (#emph[feedback]) dari purwarupa yang telah mereka buat]. Evaluasi observasional ini memberi para insinyur pengetahuan baru yang digunakan untuk melakukan iterasi, merevisi desain, dan memperbaiki solusi teknis pada siklus pengembangan selanjutnya.
...
- #strong[Perluasan Metrik Holistik di Masa Depan:] Agar perancangan artefak PSKVE menjadi lebih presisi di masa mendatang, pengembangan model evaluasi untuk dimensi #emph[Environmental Space Energy] disarankan tidak hanya berhenti di emisi karbon. Kerangka kerja ini mengusulkan metrik yang lebih luas, seperti #strong[penilaian polusi suara (#emph[noise pollution]), efisiensi tata guna lahan (#emph[land use efficiency]), dampak visual terhadap ruang, hingga kalkulasi jejak emisi siklus hidup secara utuh---dari tahap awal manufaktur pembuatan artefak hingga fase pembuangannya].
...
- #strong[Tingkat Kesiapan Teknologi (TRL -- #emph[Technology Readiness Level])].

- #strong[Tingkat kompleksitas dari algoritma yang mendasari sistem tersebut].

- #strong[Kebutuhan data (#emph[data requirements]) yang diwajibkan agar komponen kecerdasan buatan (AI) dapat berfungsi secara maksimal].
...
- #strong[Biaya Siklus Hidup yang utuh (#emph[Full Lifecycle Costing] / LCC),] yang turut memperhitungkan biaya modal awal untuk pembuatan kendaraan dan infrastrukturnya.

- #strong[Analisis biaya-manfaat kemasyarakatan (#emph[societal cost-benefit analysis])].
...
- #strong[Perluasan Metrik Holistik di Masa Depan:] Agar perancangan sistem rekayasa menjadi lebih presisi dalam menilai kualitas lingkungan dan ruang inspirasionalnya, pengembangan kerangka kerja di masa depan mengusulkan metrik pengukuran yang jauh lebih luas. Metrik tersebut mencakup #strong[penilaian polusi suara (#emph[noise pollution]), efisiensi tata guna lahan, dampak visual dari suatu ruang, hingga kalkulasi jejak emisi siklus hidup secara utuh---sejak tahap manufaktur artefak hingga fase pembuangannya].
...
#heading(level: 1, numbering: none)[Bagian II : Metodologi Multi-Domain PICOC]
...
- #strong[Cx (#emph[Context]):] Masalah spesifik, batasan kebutuhan, atau kesenjangan pengetahuan yang menjadi latar belakang perancangan.
...
Pada lapisan teratas, yaitu #strong[Domain Aplikasi (A)], metodologi ini berfokus secara eksklusif pada upaya #strong[menemukan solusi yang lebih baik untuk memecahkan masalah nyata yang dihadapi oleh para pemangku kepentingan (#emph[stakeholders])].

Penerapan kelima komponen PICOC pada Domain Aplikasi (PICOC-A) dijabarkan sebagai berikut:

- #strong[P(A) - Populasi (#emph[Population]):] Menargetkan subjek atau kelompok yang diteliti, yang dalam domain ini berupa #strong[para pemangku kepentingan, seperti pengguna akhir, organisasi, atau masyarakat luas].

- #strong[I(A) - Intervensi (#emph[Intervention]):] Merepresentasikan #strong[solusi baru yang diusulkan] kepada masyarakat. Solusi pada tingkat aplikasi ini merupakan manifestasi langsung dari sistem cerdas yang telah dirancang pada Domain Sistem (S).

- #strong[C(A) - Kontrol (#emph[Control]):] Merupakan #strong[solusi lama, sistem eksisting, atau praktik yang saat ini digunakan] oleh populasi. Analisis pada bagian ini mencakup evaluasi terhadap batasan, kelemahan, dan alasan mengapa solusi lama tersebut sudah tidak lagi memadai.
...
- #strong[Cx(A) - Konteks (#emph[Context]):] Mendefinisikan #strong[masalah spesifik yang melatarbelakangi perancangan], beserta spesifikasi kebutuhan (#emph[requirements]) yang harus dipenuhi agar solusi tersebut dapat dikatakan berhasil memecahkan masalah pemangku kepentingan.
...
- #strong[Definisi dan Wujud Stakeholder (P(A)):] Stakeholder dapat berupa #strong[pengguna akhir (#emph[users]), organisasi, institusi, masyarakat luas, hingga para insinyur itu sendiri]. Sebagai contoh kasus ekstrem, Perserikatan Bangsa-Bangsa (PBB) dapat bertindak sebagai stakeholder (P) dalam proyek pengiriman pesan ke galaksi lain. Dalam proyek rekayasa perangkat lunak, para insinyur yang membutuhkan alat desain otomatis juga merupakan wujud dari stakeholder. Jika pengujian aplikasi melibatkan manusia secara langsung, definisi P(A) ini harus mencakup rincian seperti kriteria seleksi, ukuran sampel, dan demografi partisipan.
...
Dalam metodologi pelaporan PICOC (#emph[Population, Intervention, Control, Outcome, Context]) yang diterapkan pada Domain Aplikasi (A), komponen #strong[Cx (Konteks) secara spesifik merujuk pada rumusan masalah, batasan (#emph[constraints]), lingkungan, serta spesifikasi kebutuhan (#emph[requirements]) yang harus diselesaikan untuk para pemangku kepentingan (#emph[stakeholders])].
...
- #strong[Sebagai Titik Tolak dan Pernyataan Masalah (#emph[Problem Statement]):] Cx(A) merupakan fondasi utama yang memotivasi seluruh proses rekayasa. Pada tahap ini, insinyur harus mendefinisikan secara jelas apa yang menjadi masalah utama pemangku kepentingan (P(A)), serta #strong[mengidentifikasi kesenjangan (#emph[gap]) antara kondisi ideal yang mereka butuhkan dengan ketersediaan solusi pada saat ini].
...
- #strong[Memberikan Batasan dalam Metodologi Pengujian:] Ketika insinyur merancang pengaturan eksperimen di Domain Aplikasi, komponen Cx(A) dijabarkan kembali untuk #strong[merinci latar atau pengaturan spesifik (#emph[setting]) serta batasan dari pengujian aplikasi tersebut]. Ini memastikan bahwa solusi yang diuji berada pada lingkungan yang mencerminkan realitas permasalahan yang dihadapi pengguna.
...
- #strong[C(S) - Kontrol (#emph[Control]):] Merupakan #strong[sistem dasar (#emph[baseline]) atau sistem eksisting (lama) yang digunakan sebagai pembanding]. Pada perancangan platform perangkat lunak cerdas, C(S) adalah sistem Prolog mandiri atau skrip Python tradisional yang belum terintegrasi satu sama lain.

- #strong[O(S) - Hasil (#emph[Outcome]):] Mengukur efektivitas operasional dari sistem yang dirancang melalui metrik performa tingkat sistem. Metrik pengukurannya mencakup #strong[tingkat akurasi, kecepatan (#emph[speed]), pemanfaatan sumber daya (#emph[resource utilization]), tingkat keandalan (#emph[reliability]), hingga fleksibilitas alur kerja].

- #strong[Cx(S) - Konteks (#emph[Context]):] Mendefinisikan #strong[kebutuhan spesifik akan sebuah sistem yang lebih baik], yang mampu memenuhi prasyarat solusi aplikasi sekaligus mewadahi prasyarat teknologi dasar di baliknya. Ini juga merinci latar belakang batasan desain dari sistem tersebut.
...
- #strong[P(T) - Populasi (#emph[Population]):] Dalam domain ini, populasi yang menjadi subjek perlakuan atau pemrosesan bukanlah manusia atau sistem utuh, melainkan #strong[energi sumber, nilai input, data mentah, atau material] yang akan diproses oleh mesin tersebut.

- #strong[I(T) - Intervensi (#emph[Intervention]):] Merepresentasikan #strong[Mesin atau Modul Teknologi (#emph[Technological Engine/Module]) dengan instrumen atau metode baru yang diusulkan]. Dalam paradigma #emph[Smart Engineering], intervensi teknologi ini kerap kali diabstraksikan sebagai #emph[Smart Engine] yang mengadopsi siklus kecerdasan PUDAL untuk terus mengoptimalkan konversi energi di dalamnya.

- #strong[C(T) - Kontrol (#emph[Control]):] Merupakan #strong[mesin, modul teknologi, instrumen, atau metode lama yang saat ini eksis], yang digunakan sebagai garis dasar (#emph[baseline]) pembanding untuk menguji efisiensi inovasi baru.
...
- #strong[Cx(T) - Konteks (#emph[Context]):] Mendefinisikan #strong[tantangan spesifik teknis, sasaran, maupun batasan] dalam mencari instrumen yang tepat untuk menjalankan proses konversi energi/tugas komputasi dengan baik.
...
- #strong[Penyokong Utama Kinerja Sistem (S):] Hasil peningkatan performa pada mesin atau modul teknologi (O(T)) menjadi #strong[prasyarat mutlak atau faktor pemungkin (#emph[enabler]) bagi beroperasinya arsitektur sistem secara keseluruhan di Domain Sistem (O(S))]. Sebuah sistem cerdas hanya dapat memproses alur kerja dengan optimal apabila komponen mesin penggeraknya memiliki konversi energi yang berefisiensi tinggi.
...
Dalam metodologi pelaporan PICOC yang diterapkan pada tingkat Domain Teknologi (PICOC-T), komponen #strong[P (Populasi) tidak merujuk pada manusia, pemangku kepentingan, ataupun sistem yang sudah utuh. Sebaliknya, komponen ini direpresentasikan secara spesifik sebagai Energi Sumber (#emph[Source Energy]), Nilai Input, Data Mentah (#emph[Raw Data]), Fenomena, atau Material dasar].
...
Dalam metodologi pelaporan PICOC yang diterapkan pada Domain Teknologi (PICOC-T), komponen #strong[I (Intervensi)] secara spesifik merujuk pada #strong[Mesin Teknologi (#emph[Technological Engine]) atau Modul dengan instrumen maupun metode yang baru].
...
- #strong[P(F) - Populasi (#emph[Population]):] Subjek yang diteliti pada level ini sangat abstrak, yakni berupa #strong[fenomena alam atau konseptual, entitas-entitas fundamental, serta nilai atau energi sumber yang sedang diinvestigasi].

- #strong[I(F) - Intervensi (#emph[Intervention]):] Merepresentasikan gagasan fundamental yang baru, yang dapat berupa #strong[proses, teori, kerangka pemodelan (#emph[model]), maupun pendekatan eksperimental baru] yang diusulkan oleh peneliti.

- #strong[C(F) - Kontrol (#emph[Control]):] Merupakan #strong[teori, model, pendekatan, atau proses eksisting (lama)] yang digunakan sebagai landasan pembanding untuk menguji keabsahan teori baru tersebut.
...
#heading(level: 1, numbering: none)[Bagian III: Integrasi Teknologi]
...
#strong[Representasi Fakta (#emph[Facts])] Fakta adalah #strong[pernyataan deklaratif yang menegaskan kebenaran tanpa syarat (kebenaran absolut) tentang objek di dalam domain masalah]. Dalam pemodelan ontologi rekayasa cerdas, fakta digunakan untuk mendeklarasikan entitas-entitas dasar beserta atributnya:
...
- #strong[Mewujudkan Hierarki dan Pewarisan (#emph[Inheritance]):] Aturan digunakan untuk memetakan hierarki kelas. Aturan #NormalTok("mammal(X) :- cat(X)."); menetapkan logika bahwa siapa pun (X) yang berstatus sebagai kucing, secara otomatis mewarisi identitas sebagai mamalia.
...
- #strong[Pembuktian Teorema Resolusi (#emph[Resolution Theorem Prover]):] Saat pengguna atau sistem mengajukan kueri (pertanyaan), mesin inferensi berfungsi sebagai pembukti teorema otomatis yang mencari tahu apakah pernyataan tersebut benar berdasarkan basis pengetahuan yang tersedia.
...
- #strong[Unifikasi (#emph[Unification]):] Mekanisme ini merupakan fondasi di mana mesin secara otomatis mencocokkan pola variabel dalam kueri dengan struktur fakta atau aturan yang ada di dalam ontologi.

#strong[Beroperasi dengan Asumsi Dunia Tertutup (#emph[Closed World Assumption])] Karakteristik fundamental dari mesin inferensi Prolog adalah ia beroperasi di bawah prinsip #emph[Closed World Assumption] (CWA). Prinsip ini menetapkan bahwa #strong[jika mesin inferensi tidak dapat membuktikan suatu pernyataan bernilai benar dari fakta dan aturan yang ada, maka pernyataan tersebut diasumsikan mutlak salah] (juga dikenal sebagai #emph[Negation as Failure]). Walaupun sangat efisien untuk batas domain yang pasti, sifat CWA ini menjadi tantangan tersendiri ketika Prolog digunakan untuk merancang sistem yang menghadapi informasi yang tidak lengkap dari dunia nyata.

#strong[Peran Sentral dalam Integrasi Sistem Rekayasa (#emph[Smart Engineering])] Di dalam perancangan platform #emph[Smart Engineering] modern, kekuatan mesin inferensi ini diisolasi untuk fokus pada hal yang paling ia kuasai, lalu diintegrasikan dengan bahasa multiguna seperti Python.
...
#strong[\3. Pertukaran dan konversi tipe data (#emph[data marshalling]).] Salah satu tantangan terbesar dalam pemrosesan data lintas bahasa adalah mengelola aliran data antara domain komputasi (Python) dan domain simbolis (Prolog). Integrasi ini bergantung pada modul jembatan (#emph[bridge libraries] seperti PySWIP atau Janus) yang memfasilitasi komunikasi data secara transparan. Pustaka ini #strong[bertanggung jawab secara otomatis mengonversi struktur data Python (seperti #emph[string], #emph[integer], #emph[float], #emph[list], dan #emph[dictionary]) menjadi tipe data Prolog (#emph[atom], #emph[number], #emph[term], dan #emph[list])], dan begitu pula sebaliknya saat menerima hasil evaluasi dari Prolog. Konversi yang mulus ini membebaskan insinyur dari kerumitan melakukan serialisasi data secara manual.
...
Dalam arsitektur terintegrasi Prolog-Python untuk platform #emph[Smart Engineering], #strong[Python secara eksklusif mengemban tanggung jawab pada lapisan aplikasi, yang salah satu fokus utamanya adalah pengembangan Antarmuka Pengguna (#emph[User Interface] / UI) serta interaksi pengguna].
...
- #strong[Pengelola Interaksi dan Alur Kerja Utama:] Aplikasi yang dibangun dengan Python bertugas untuk #strong[mengelola seluruh interaksi pengguna, mengumpulkan input atau parameter awal, dan mengatur alur kendali (#emph[control flow])]. Ketika pengguna memberikan perintah melalui antarmuka Python, aplikasi akan meneruskan data tersebut kepada mesin Prolog untuk dievaluasi logika atau kendalanya (#emph[constraints]), untuk kemudian ditarik kembali hasil inferensinya.
...
- #strong[Pemanfaatan Ekosistem Pustaka (#emph[Libraries]) yang Kaya:] Fleksibilitas Python menjadikannya pilihan ideal untuk pengembangan aplikasi karena ia didukung oleh ekosistem pustaka yang sangat luas yang mencakup pengembangan web, visualisasi data, dan Antarmuka Pengguna Grafis (GUI).
...
- #strong[Penggerak Simulasi Dinamis (#emph[Dynamic State Modification]):] Sebuah simulasi yang sesungguhnya harus mampu merepresentasikan perubahan status seiring berjalannya waktu. Python mengorkestrasi dinamika ini dengan #strong[secara aktif menyuntikkan (meng-#emph[assert]) fakta-fakta baru ke dalam basis pengetahuan Prolog saat simulasi sedang berjalan (#emph[runtime]), serta menariknya kembali (#emph[retract]) ketika status tersebut sudah tidak relevan]. Kemampuan manipulasi interaktif inilah yang memungkinkan status dari model Prolog untuk terus berevolusi merespons peristiwa dan data skenario tanpa harus memodifikasi kode dasar Prolog untuk setiap skenario yang berbeda.
...
#strong[\1. Manajemen Konversi Tipe Data yang Transparan (#emph[Data Marshalling])] Komunikasi yang efektif antara Python dan Prolog sangat bergantung pada kemampuan untuk mengonversi tipe data secara mulus. #emph[Bridge libraries] memikul tanggung jawab krusial ini dengan memetakan struktur data secara otomatis tanpa mengharuskan insinyur melakukan serialisasi data secara manual. Sebagai contoh, pustaka ini secara otomatis mengubah #emph[string], #emph[integer], #emph[float], dan #emph[list] dari Python menjadi bentuk #emph[atom], #emph[number], dan #emph[term] yang dipahami Prolog, lalu mengonversi kembali variabel hasil deduksi Prolog menjadi struktur seperti #emph[dictionary] di dalam Python.

#strong[\2. Menjembatani Orkestrasi Alur Kerja (#emph[Calling Conventions])] Pustaka ini menyediakan antarmuka teknis yang mengizinkan Python untuk sepenuhnya mengendalikan dan berinteraksi dengan mesin Prolog. Melalui pustaka ini, skrip Python dapat memulai sesi Prolog, memuat berkas ontologi (#NormalTok(".pl");), secara dinamis menyuntikkan atau menarik fakta saat simulasi berjalan (seperti #NormalTok("assertz"); atau #NormalTok("retract");), serta mengirimkan rangkaian kueri logika untuk dievaluasi oleh mesin inferensi Prolog.
...
- #strong[Manajemen Konversi Data (#emph[Data Marshalling]) yang Sangat Rinci:] Keberhasilan integrasi sistem sangat bergantung pada kemulusan konversi tipe data. Janus menyembunyikan kerumitan serialisasi dengan menyediakan peta terjemahan tipe data transparan yang komprehensif :
...
#heading(level: 1, numbering: none)[Bagian IV : Studi Kasus]
...
- #strong[Energi Layanan (#emph[Service Energy]):] Direpresentasikan secara objektif melalui efisiensi waktu atau estimasi waktu tempuh perjalanan, di mana Kereta Cepat menunjukkan performa Energi Layanan terbaik.

- #strong[Energi Nilai (#emph[Value Energy]):] Diukur melalui total biaya yang harus dikeluarkan per penumpang. Parameter algoritmik di Python mengakumulasikan biaya ini berdasarkan harga energi, tarif tol, nilai tiket, hingga pro-rata biaya pemeliharaan mesin.

- #strong[Energi Ruang Lingkungan (#emph[Environmental Space Energy]):] Mewakili jejak ekologis dari teknologi tersebut, yang dalam simulasi ini dikalkulasi sebagai jumlah gram emisi CO2 ekuivalen per penumpang.
...
- #strong[Beban Permintaan (#emph[Passenger Demand]):] Skenario ini menargetkan pengangkutan sebuah kelompok yang terdiri dari #strong[100 orang penumpang] secara bersamaan.
...
- #strong[Armada Sedan Listrik (#emph[Electric Sedan]):] Memiliki kapasitas 4 penumpang per unit, sehingga skenario ini membutuhkan kalkulasi pengoperasian 25 unit mobil listrik.

- #strong[Armada Bus Diesel Euro 6 (#emph[Biodiesel]):] Memiliki kapasitas 40 penumpang per unit, sehingga skenario ini membutuhkan pengoperasian 3 unit bus.

- #strong[Kereta Cepat Listrik (#emph[High-Speed Train]):] Memiliki kapasitas masif sebesar 500 penumpang per rangkaian, sehingga cukup menggunakan satu layanan untuk mengangkut seluruh kelompok.
...
- #strong[Energi Layanan (#emph[Service Energy]):] Direpresentasikan melalui estimasi waktu tempuh. Kereta Cepat menawarkan efisiensi energi layanan terbaik (sekitar 1 hingga 1,5 jam) dibandingkan opsi jalan raya.
...
position: top, 
[
Skenario Perjalanan: Moda, Asumsi, Jarak, Waktu, dan Hambatan
]), 
kind: "quarto-float-tbl", 
supplement: "Tabel", 
)
<tbl-skenario-perjalanan>
...
position: top, 
[
Komparatif EV, Bus, dan Kereta Cepat menurut Beberapa Metrik Kunci
]), 
kind: "quarto-float-tbl", 
supplement: "Tabel", 
)
<tbl-komparatif-kendaraan>
...
#strong[\1. Metrik Waktu Tempuh: Representasi Energi Layanan (#emph[Service Energy])] Waktu tempuh dievaluasi untuk mengukur efisiensi layanan dari sebuah sistem transportasi.
...
#strong[\3. Metrik Emisi CO2: Representasi Energi Ruang Lingkungan (#emph[Environmental Space Energy])] Metrik ini mengukur jejak karbon dalam wujud gram ekuivalen CO2 per penumpang untuk merepresentasikan dampak ekologis sistem transportasi.
...
#strong[Signifikansi dalam Kerangka] #strong[Smart Engineering] Perhitungan ketiga metrik ini digunakan untuk memvalidasi prinsip #strong[Konversi Transaksional (#emph[Transactional Conversion])]. Melalui komparasi simulasi ini, sistem #emph[Smart Engineering] mengilustrasikan bagaimana seorang pengguna (penumpang) melakukan transaksi energi dimensi silang di dunia nyata: #strong[mereka menukarkan Energi Nilai (biaya operasional atau tiket) untuk mendapatkan Energi Layanan (efisiensi waktu tempuh) dengan mempekerjakan Energi Produk mekanis kendaraan, yang keseluruhannya meninggalkan jejak Energi Ruang Lingkungan (emisi CO2)].
...
position: top, 
[
Metrik Inti, Definisi, Satuan, Arah Preferensi, dan Sumber Data
]), 
kind: "quarto-float-tbl", 
supplement: "Tabel", 
)
