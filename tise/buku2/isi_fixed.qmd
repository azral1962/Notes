...
Dalam arsitektur *Smart Engineering* yang terintegrasi, **Python mengambil peran sentral sebagai orkestrator yang mengendalikan keseluruhan alur kerja (****workflow control****), khususnya dalam menjalankan dan mengelola sebuah simulasi** [@ontology_prolog_python] [@prolog_python_integration] .

Sementara Prolog beroperasi sebagai mesin logika dan basis pengetahuan statis di latar belakang, Python memberikan kontrol aliran imperatif (*imperative control flow*) serta kemampuan komputasi numerik yang "menghidupkan" ontologi tersebut menjadi sebuah simulasi yang berjalan [@smart_engineering_framework] .

Berikut adalah penjabaran mengenai bagaimana Python melakukan orkestrasi simulasi:

-   **Inisialisasi dan Persiapan Skenario:** Skrip Python mengorkestrasi tahap awal simulasi dengan menginisialisasi instansiasi mesin Prolog dan memuat berkas basis pengetahuan (misalnya file `.pl` yang berisi ontologi rekayasa) ke dalam memori [@prolog_python_integration] [@ontology_prolog_python] . Pada tahap ini, Python juga bertugas mendefinisikan parameter skenario, seperti menentukan input pengguna atau kondisi awal lingkungan simulasi [@smart_engineering_framework] .

-   **Penggerak Simulasi Dinamis (****Dynamic State Modification****):** Simulasi pada dasarnya adalah tentang merepresentasikan perubahan status seiring berjalannya waktu atau urutan peristiwa. Python mengorkestrasi dinamika ini dengan **secara aktif menyuntikkan (meng-****assert****) fakta-fakta baru ke dalam mesin Prolog saat aplikasi berjalan (****runtime****), dan menariknya kembali (****retract****) ketika status tersebut sudah tidak relevan** [@ontology_prolog_python] . Kemampuan untuk memanipulasi fakta secara dinamis dari Python ini sangat vital untuk menciptakan simulasi interaktif, di mana status "dunia" di dalam Prolog dapat berevolusi merespons peristiwa *real-time* tanpa harus mengubah kode sumber Prolog aslinya [@prolog_python_integration] .

-   **Mesin Kalkulasi dan Eksekusi Kueri Iteratif:** Setelah parameter skenario ditetapkan, Python akan secara iteratif menyusun kueri untuk dikirimkan ke Prolog. Prolog memproses logika tersebut, lalu Python menarik kembali solusinya [@prolog_python_integration] . Sebagai contoh, dalam studi kasus simulasi transportasi rute Bandung-Jakarta 2030, **kelas-kelas Python bertindak sebagai mesin kalkulasi utama yang mengambil data spesifikasi kendaraan dari ontologi Prolog, lalu menggunakan algoritma algoritmik murni untuk menghitung total kebutuhan unit, kalkulasi biaya operasional, dan proyeksi emisi CO2 untuk 100 penumpang** [@smart_engineering_framework] .

-   **Presentasi dan Evaluasi Hasil:** Setelah iterasi simulasi selesai, Python mengambil seluruh hasil komputasi (baik deduksi logis dari Prolog maupun kalkulasi numerik dari algoritmanya sendiri) dan memprosesnya menjadi *output* yang bermakna [@prolog_python_integration] [@ontology_prolog_python] . Python bertugas untuk menyajikan komparasi alternatif solusi tersebut kepada pengguna atau menggunakannya untuk evaluasi keputusan lebih lanjut [@smart_engineering_framework] .

Secara keseluruhan, **kemampuan orkestrasi Python mengubah model ontologi yang kaku menjadi aplikasi dan simulasi rekayasa yang dinamis, interaktif, dan sadar-pengetahuan (****knowledge-aware****)** [@prolog_python_integration] [@ontology_prolog_python] . Penyatuan orkestrasi algoritmik di Python dengan representasi pengetahuan di Prolog ini menciptakan metodologi yang sangat tangguh untuk memodelkan sistem multi-domain yang kompleks [@smart_engineering_framework] .
...
-   **Rute:** Perjalanan dari koridor Bandung menuju Jakarta dengan jarak tempuh 150 km [@smart_engineering_framework] .

-   **Tahun Proyeksi:** Kondisi teknologi dan lingkungan (seperti ketersediaan jaringan listrik dan harga energi) disimulasikan untuk tahun 2030 [@smart_engineering_framework] .

-   **Beban Permintaan:** Skenario ini menargetkan pengangkutan sebuah kelompok yang terdiri dari **100 orang penumpang** secara bersamaan [@smart_engineering_framework] .
...
-   **Armada Sedan Listrik (****Electric Sedan****):** Memiliki kapasitas 4 penumpang per unit, sehingga sistem (Python) menghitung kebutuhan sebanyak 25 unit mobil listrik untuk skenario ini [@smart_engineering_framework] .

-   **Armada Bus Diesel Euro 6 (****Biodiesel****):** Memiliki kapasitas 40 penumpang per unit, sehingga sistem menghitung bahwa skenario ini membutuhkan pengoperasian 3 unit bus [@smart_engineering_framework] .

-   **Kereta Cepat Listrik (****High-Speed Train****):** Memiliki kapasitas masif sebesar 500 penumpang per rangkaian, sehingga dapat mengangkut seluruh kelompok penumpang secara sekaligus [@smart_engineering_framework] .
...
-   **Energi Layanan (****Service Energy****):** Diukur secara objektif melalui estimasi waktu tempuh perjalanan. Dalam skenario ini, Kereta Cepat menawarkan efisiensi energi layanan terbaik (sekitar 1 hingga 1,5 jam) dibandingkan alternatif jalan raya [@smart_engineering_framework] .

-   **Energi Nilai (****Value Energy****):** Diukur berdasarkan akumulasi total biaya per penumpang. Python menghitung hal ini dengan menjumlahkan tarif energi/bahan bakar, biaya tol, hingga prorata biaya pemeliharaan tahunan. Secara hipotesis, armada bus menunjukkan biaya termurah per penumpang, diikuti armada Sedan EV, sementara Kereta Cepat menuntut pengeluaran biaya (harga tiket) tertinggi secara langsung [@smart_engineering_framework] .

-   **Energi Ruang Lingkungan (****Environmental Space Energy****):** Diukur dari total dampak emisi CO2 ekuivalen per penumpang. Kereta cepat menghasilkan emisi karbon terendah, sementara emisi Sedan EV sangat bergantung pada tingkat kebersihan sumber jaringan listrik (PLN) di tahun 2030, dan armada bus mencatatkan hasil polusi tertinggi [@smart_engineering_framework] .

**Signifikansi dalam Konteks** **Smart Engineering** Skenario spesifik Bandung-Jakarta ini menjadi pembuktian konsep terkait **Konversi Transaksional (****Transactional Conversion****)**. Skenario ini mengilustrasikan bagaimana sebuah ekosistem bekerja ketika pengguna melakukan transaksi lintas-dimensi: mereka menukarkan Energi Nilai (berupa uang/biaya operasional) untuk mendapatkan Energi Layanan (efisiensi waktu transportasi) dengan mempekerjakan Energi Produk mekanis (kendaraan) [@smart_engineering_framework] .

Pada akhirnya, pendelegasian ontologi kendaraan di dalam Prolog yang dieksekusi simulasi perhitungannya oleh Python terbukti mampu menopang insinyur untuk menganalisis parameter kompleks dari berbagai sistem (perangkat lunak, perangkat keras, dan infrastruktur energi) secara sekaligus untuk pengambilan keputusan rekayasa yang lebih holistik [@smart_engineering_framework] .
