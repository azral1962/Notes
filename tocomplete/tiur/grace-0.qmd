...
funding: 
  statement: "Manuscript created May, 2026."
pageheader:
  left: IEEE Transactions on Technology and Society, Month Year
  right: 'T. Gantini et.al.:  GRACE: Ecosystem for Elderly'
bibliography: refs.bib
---

# Introduction {#sec-intro}

[P]{.IEEEPARstart}[opulation]{} aging is reshaping healthcare, labor, family structures, and public services. In response, digital systems for older adults have grown rapidly, especially in telehealth, remote monitoring, medication support, and care coordination. These systems are valuable, but many are built around narrow assumptions: that old age is primarily a condition of decline, risk, and dependency.
...
: First Table {#tbl-first, tbl-colwidths="[20,26, 26, 26]"}
...
```{r}
#| label: tbl-wrapped
#| tbl-cap: "A nicely wrapped table using kableExtra"

library(knitr)
library(kableExtra)

# Sample data with long strings
df <- data.frame(
  Doman = c("Very long introductory sentence that will wrap", "Short text"),
  Description = c("Another long explanation for a completely different scenario.", "Simple detail"),
  Functions = c("Very long introductory sentence that will wrap", "Short text"),
  Examples = c("Another long explanation for a completely different scenario.", "Simple detail")
)

# Render and wrap columns
kbl(df) |>
  kable_styling(bootstrap_options = c("condensed")) |>
  column_spec(1, width = "5em") |>   # Set width for column 1
  column_spec(2, width = "6.5em") |>   # Set width for column 1
  column_spec(3, width = "6.5em") |>   # Set width for column 1
  column_spec(4, width = "6.5em")      # Set width for column 2
```
...
```{r}
#| label: tbl-domain
#| tbl-cap: "Domain, description, wellbuing functions, adn examples system supports."
#| tbl.env: tabular
#| fig-pos: h

options(knitr.table.format = function() {if (knitr::is_latex_output()) 'latex' else 'pandoc'})
dt <- data.frame(Domain=c('Spiritual \ Self','Family and Friends','Working Colleagues', 'Communities','Wider World'),
                 Description=c('Inner life, values, faith, reflection, existential orientation','Intimate and enduring personal relationships','Ongoing or legacy vocational and professional ties', 'Local groups, associations, faith communities, neighborhoods, civic groups', 'Markets, institutions, services, transport, civic systems, public services '),
                 Functions=c('D','E','F', '4', '5'),
                 Examples=c('G','H','I', '4', '5'))
...
The GRACE Index model is summarized visually in Fig. 5, with dimensions and data sources in Table III and scoring interpretation in Table IV.
...
::: {#tbl-zone layout-ncol=2}
| Component / Zone | Weight / Range | Interpretation |
|---|---|---|
...
A person may remain in a stable narrative zone even during distress if selfhood, belonging, and meaning remain intact.

## V. Three AI-Enabled Innovations

These innovations are summarized in Table V. The hero’s-journey structure is shown in Fig. 4, while the predictive-intervention loop is shown in Fig. 3.

::: {#tbl-innovations layout-ncol=2}
...
The architecture is shown in Fig. 2. Agent responsibilities are summarized in Table VI. Governance risks and safeguards are summarized in Table VII and visually in Fig. 7.

::: {#tbl-agents layout-ncol=2}
...
Because GRACE handles intimate autobiographical and relational data, governance is central. The system should include consent-based data access, explainable interventions, adjustable autonomy, human override, privacy protection, and safeguards against manipulative prompting.

## VII. Agent-Based Simulation Design

The simulation structure is shown in Fig. 6, with conditions and hypotheses in Table VIII and variables in Table IX.
...
::: {#tbl-panel layout-ncol=2}
| Condition / Hypothesis | Story | Prediction | Stimulus | Expectation |
|---|---|---|---|---|
...
Taken together, Figs. 1–7 and Tables I–IX show that GRACE is not merely a collection of tools, but an integrated socio-technical ecosystem.
